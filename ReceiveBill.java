...
public class ReceiveBill extends JFrame implements MouseListener
{
	
	private JLabel LabelTitle,WelcomeLabel, LabelMemberID, LabelBill,LabelBackGroundImage;
	private JLabel LabelPaidDate, LabelPDD, LabelPMM, LabelPYYYY;
	private JTextField MemberIDTF, MemberBillTF;
	...
	private int AdminId, MemberId, count;
	private boolean flagTransactionDate=true, flagDB,flagCount;
	
	public ReceiveBill(int AdminId)
	{
	super("Recive Bill");
	...
	LabelTitle = new JLabel("Receive Member Bill");
	...
	LabelMemberID.setBounds(120, 250, 150, 25);
	...
	MemberIDTF.setBounds(300, 250, 135, 25);
	panel.add(MemberIDTF);
	
	LabelMemberID = new JLabel("Enter Amount        :");
	LabelMemberID.setBounds(120, 290, 150, 25);
	LabelMemberID.setFont(new Font("Tahoma", Font.PLAIN, 15));
	LabelMemberID.setForeground(Color.BLACK);
	panel.add(LabelMemberID);

	MemberBillTF = new JTextField();
	MemberBillTF.setBounds(300, 290, 135, 25);
	panel.add(MemberBillTF);
	...
	buttonReciveBill = new JButton("Receive Bill");
	...
	public void mouseClicked(MouseEvent me)
	{
		...
		{
		if(MemberBillTF.getText().isEmpty())
	    {
        ...
        Fee=Double.parseDouble(MemberBillTF.getText());	
		}catch(Exception e){}		
		getPaidDate();
	...
	{   
		String queryTransactionDate ="SELECT `AccountId`, `Status` ,`Month` , `Year` FROM `transaction` WHERE AccountId ="+ MemberId+";"; 
	    Connection con=null;
        Statement st = null;
		ResultSet rs = null;		
			...
			while(rs.next())
			{
				...
				if(MemberId==id && PMonth==m && PYear==y)
	...
	{   
		...
		ResultSet rs = null;		
			...
			queryTransaction = "INSERT INTO transaction VALUES ("+transactionNo+","+MemberId+",'"+Status+"',"+Fee+","+Salary+","+PMonth+","+PYear+",'"+PaidDate+"',"+AdminId+");"; 
	...
	public void UpdateAccount()
	{
	        Connection con =null;
            Statement st = null;
			String queryForUpdate = "UPDATE accounts SET Validity= 'Valid' where UserId="+MemberId;
		...
		try
		{
			...
			JOptionPane.showMessageDialog(this, "Account is Now valid"); 
			st.close();
			con.close();
		}
		catch(Exception e)
