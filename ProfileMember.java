...
public class ProfileMember extends JFrame implements ActionListener, MouseListener
{
	...
	public ProfileMember(int MemberId)
	{
    ...
    this.setIconImage(img.getImage());			
		...
		WorkoutTitle = new JLabel("Member Profile");
		...
		LabelMemberId = new JLabel("ID                : "+MemberId);
		...
		String[] columns = {"Transaction No","Paid( Tk.)","Date","Received By"};
		String[][] data = {{"",   "",   "",    ""},
	...
	public void mouseClicked(MouseEvent me)
	{
		...
		{
			MemberRules mr = new MemberRules(MemberId);
			try{
			this.setVisible(false);
			mr.setVisible(true); 	
			}catch(Exception ex){}
	...
	{
        String query = "SELECT `TransactionNo`, `ReceiveBill`, `TransactionDate`, `AdminId` FROM `transaction`  WHERE AccountId ="+ MemberId+" ORDER BY TransactionNo DESC LIMIT 1;";      
        Connection con=null; 
        Statement st = null; 
		ResultSet rs = null; 
	...
	{
        String query = "SELECT `Name`, `Gender` , `DOB`, `Photo`,`AdmissionDate` FROM `memberacc` WHERE UserId ="+ MemberId +";";        
        Connection con=null; 
        Statement st = null; 
		ResultSet rs = null; 
		System.out.println(query);
        try
		{
			...
			while(rs.next())
			{
				...
				String a = rs.getString("AdmissionDate");
